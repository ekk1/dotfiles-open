...
pp.add_argument('-p', '--proxy',    action="store_true", help="assume need proxy")
...
_aptproxy = " -p=1" if aa.proxy else ""
...
for _vm_no in range(0, _multi_qemu):
    ...
    init_cmd =  f"ssh -R 9050 -p {2221 + _vm_no} debian@127.0.0.1 -i vm_key sudo bash 93.init.debian.root.sh -a={aa.apt}{_nohttps}{_aptproxy}"
    if _inner_addr_base != "":
        run_cmd(f"{init_cmd} -i={_inner_addr_base}{_inner_addr_start + _vm_no}", dry_run=aa.dry)
    else:
        run_cmd(f"{init_cmd}", dry_run=aa.dry)
    if aa.proxy:
